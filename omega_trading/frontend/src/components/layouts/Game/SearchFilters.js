...
function SearchFilters(props) {
  ...
  if (amounts.includes(amount)) {
   return (
    <button
     ...
     className='checked fr jc-c ai-c f18 bld'
    >
     &#10003;
    </button>
   );
 ...
 const commission_selected = (commission) => {
  if (commissions.includes(commission)) {
   return (
    <button
     onClick={() => commissionClick(commission)}
     className='checked fr jc-c ai-c f18 bld'
    >
     &#10003;
    </button>
   ...
   return (
    <button onClick={() => commissionClick(commission)} className='check-box' />
   );
  }
 };
   ...
   <div className='smx smy'>
    <div className='fr jc-s ai-c'>
     <div className='fr ai-c jc-s'>
      ...
      {duration ? (
       <button
        onClick={() => handleNone(false)}
        className='checked sml fr jc-c ai-c f18 bld'
       >
        &#10003;
       </button>
      ) : (
     ...
     <div className='fr ai-c'>
      {!duration ? null : showDuration ? (
       <div className='fr ai-c'>
        <button className='sml' onClick={() => setShowDuration(false)}>
         {dropper_up}
        </button>
       </div>
      ) : (
      ...
      <div className='duration-filter fr ai-c jc-s'>
       <div className='fc ai-c'>
        <div className='duration'>
         <button onClick={() => setAnyDay(!anyDay)}>
          {anyDay ? "Any" : "Days"}
         </button>
        </div>
        {anyDay ? (
         <div className='anyHour' />
        ) : (
         <input
       ...
       <div className='fc ai-c smx'>
        <div className='duration'>
         <button onClick={() => setAnyHour(!anyHour)}>
          {anyHour ? "Any" : "Hours"}
         </button>
        </div>
        {anyHour ? (
       ...
       <div className='fc ai-c'>
        <div className='duration'>
         <button onClick={() => setAnyMin(!anyMin)}>
          {anyMin ? "Any" : "Mins"}
         </button>
        </div>
        {anyMin ? (
         <div className='anyMin' />
        ) : (
         <select
          className='min-input'
          placeholder='0'
         ...
         >
          {minutes.map((min) => {
           return (
            <option className='min-option' value={min}>
             {min}
            </option>
           );
          })}
   ...
   <div>
    <div className='smx smy'>
     <div className='fr jc-s ai-c'>
      ...
      {!bet ? null : showBet ? (
       <div className='fr ai-c'>
        <button className='sml' onClick={() => setShowBet(false)}>
         {dropper_up}
        </button>
       </div>
      ) : (
   ...
   <div className='smx smy'>
    <div className='fr jc-s ai-c'>
     ...
     <div className='fr ai-c'>
      {!date ? null : showDate ? (
       <div className='fr ai-c'>
        <button className='sml' onClick={() => setShowDate(false)}>
         {dropper_up}
        </button>
       </div>
      ) : (
    ...
    {showDate && date ? (
     <div className='fr jc-s ai-c'>
      {anyDate ? (
       <div className='anyDate smmt' />
      ) : (
       <input
        onChange={(e) => setDateMetric(e.target.value)}
        type='date'
        className='date-filter smmt'
        value={dateMetric}
        min={minDate}
        max={max}
       />
 ...
 return (
  <div className='filters br fc jc-s ai-s'>
   <div className='filter-choices'>
    <div className='smx smy'>
     <div className='fr jc-s ai-c'>
      ...
      <button onClick={() => setDropAmount(!dropAmount)}>
       {dropAmount ? dropper_up : dropper_down}
      </button>
     </div>
     <div className='fc ai-st'>{dropAmount ? get_amounts() : null}</div>
    </div>
    <div className='smx smy'>
     <div className='fr jc-s ai-c'>
      <div className='f18 bld'>Commission</div>
      <button onClick={() => setDropCommission(!dropCommission)}>
       {dropCommission ? dropper_up : dropper_down}
      </button>
     </div>
    ...
    <div className='smx smy'>
     <div className='fr jc-s ai-c'>
      ...
      <button onClick={() => setOptions(!options)} className='options-filter'>
   ...
   <div className='fr jc-c mmy'>
    <button onClick={() => makeSearch()} className='editButton'>
     {props.making_search ? <ButtonLoader /> : "Search"}
    </button>
   </div>
  </div>
 );
