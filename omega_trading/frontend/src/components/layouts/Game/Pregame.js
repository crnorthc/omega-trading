    ...
    users.push(
     <div className='fr smy smx jc-s ai-c'>
      ...
      {i in props.game.invites ? (
       ...
       <button
        onClick={() => props.invite(i, props.game.room_code)}
        className='editButtonMedium'
       >
        Invite
       </button>
      )}
   ...
   <div>
    <div className='fr mmx ai-b jc-s'>
     ...
     {props.user.username == props.game.host.username ? (
      <button
       onClick={() => props.changeType(props.game.room_code)}
       className='editButton'
      >
       {props.game.type ? "Public" : "Private"}
      </button>
     ) : (
      <button
       onClick={() => props.leaveGame(props.game.room_code)}
       className='editButton'
      >
       Leave
      </button>
    ...
    <div className='rules_container bg-main-nav fr jc-c ai-c mmt b'>
     {edit ? (
      <NewEditRules edit={handleEdit} />
     ) : (
      <GameRules edit={handleEdit} />
     )}
    </div>
   </div>
    ...
    {Object.keys(props.game.players).length == 1 ? (
     <div className='work fc ai-c jc-c'>
      ...
      <div className='noAction'>
       There needs to be atleast two players to start a game
      </div>
     </div>
    ) : props.game.host.username == props.user.username ? (
     <div className='work fc ai-c jc-c'>
      <button
       onClick={() => props.startGame(props.game.room_code)}
       className='editButton'
      >
       Start
      </button>
