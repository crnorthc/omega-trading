...
function Pregame(props) {
    ...
    const getFriends = () => {
        ...
        if (Object.keys(props.users).length == 0) {
            return (
                <div className='fr jc-c smx smy f22 lt'>No Results</div>
            )
        }
        else {
                ...
                users.push(
                    <div className='fr smy smx jc-s ai-c'>
                        ...
                        {i in props.game.invites ?  
                            ...
                            : 
                            <button onClick={() => props.invite(i, props.game.room_code)} className='editButtonMedium'>Invite</button>
                        }                        
                    </div>
                )
            }
            ...
            <div>
                <div className='fr mmx ai-b jc-s'>
                    ...
                    {props.user.username == props.game.host.username ? 
                        <button onClick={() => props.changeType(props.game.room_code)} className='editButton'>{props.game.type ? 'Public' : 'Private'}</button>
                        :
