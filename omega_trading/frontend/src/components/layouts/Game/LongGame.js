...
function Long(props) {
    ...
    const checkDates = () => {
        ...
        const start_time = (new Date(startYear, startMonth - 1, startDay, startHour, startMin).getTime() / 1000).toFixed(0)

        const end_time = (new Date(endYear, endMonth - 1, endDay, endHour, endMin).getTime() / 1000).toFixed(0)
        ...
        return (
            <>
                <div className="z-10 absolute top-16 right-0 bottom-2/3 left-0 flex items-center justify-center">
                    <h1 className="font-mono font-medium text-white text-center text-5xl sm:text-6xl  ">Long Game</h1>
                </div>
                        ...
                        <div className="mx-auto max-w-4xl rounded-xl sm:bg-gray-800 p-4">
                            <div className="grid grid-cols-12 gap-x-4 gap-y-5 sm:gap-y-10 ">
                                <div className="col-span-12 sm:col-span-8">
                                    ...
                                    <input
                                        className="block w-full text-sm shadow-sm rounded-md h-10 px-2 border-transparent focus:outline-none focus:ring focus:ring-yellow-300"
                                ...
                                <div className="col-span-6 sm:col-span-2 flex items-left sm:items-center justify-center flex-col">
                                    <Switch.Group>
                                        <Switch.Label className="block text-lg font-medium text-white" htmlFor="">
                                                ...
                                                <Listbox.Options className="z-10 absolute w-full py-1 mt-1 overflow-auto text-base bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black  ring-opacity-5 focus:outline-none sm:text-sm">
                                                    {startAmounts.map((startAmount, startAmountIdx) => (
                                                        <Listbox.Option
                                                            key={startAmountIdx}
                                                            className={({ active }) =>
                                                                `${active ? 'text-yellow-700 bg-yellow-100' : 'text-gray-900'} cursor-pointer select-none relative py-2 pl-10 pr-4`
                                                            }
                                                            value={startAmount}
                                                        >
                                                            {({ selected, active }) => (
                                                                <>
                                                                    <span className={`${selected ? 'font-medium' : 'font-normal'} block truncate`}>{startAmount.insert}</span>
                                                                    {selected ? (
...
cursor-pointer select-none relative py-2 pl-10 pr-4`
                                                        ...
                                                        >
                                                            {({ selected, active }) => (
                                                                <>
                                                                    <span className={`${selected ? 'font-medium' : 'font-normal'} block truncate`}>{minPlayer.insert}</span>
                                                                    {selected ? (
                                                                        <span
                                                                            className={`${active ? 'text-amber-600' : 'text-amber-600'}
absolute inset-y-0 left-0 flex items-center pl-3`}
                                    ...
                                    <Tab.Group>
                                        ...
                                        <Tab.List className="flex items-start justify-start pb-2">
                                            <span className="flex shadow-md rounded-md">
                                                <Tab
                                                    className={({ selected }) =>
                                                        `${selected ? 'bg-white text-gray-900' : ' text-gray-500'}
    p-2 rounded-l-lg`
                                                    }
                                                >
                                                    Start
...
absolute inset-y-0 left-0 flex items-center pl-3`}
                        ...
                        <div className="flex items-center justify-center">
                            <button onClick={() => create()} className="block bg-yellow-500 text-gray-900 text-xl sm:text-2xl py-4 px-7 mt-10 mb-32 rounded-lg shadow-md">
                                Create Game
                            </button>
                        </div>
                    </div>
