...
function MyNavbar(props) {
    ...
    const no_game = (
        <Listbox.Option key={'bitch'} className={({ active }) => `${active ? 'text-gray-800 bg-gray-200' : 'text-gray-900'} cursor-pointer select-none relative py-2 pl-10 pr-4`} value={null}>
            {({ selected, active }) => (
                <>
                    ...
                    <div className='flex items-center h-full justify-center sm:justify-between shadow-sm'>
                            ...
                            <div className='w-40 sm:w-52 ml-5'>
                                <Listbox value={selected} onChange={setSelected}>
                                    <div className={'relative '}>
                                        <div className='flex'>
                                            <div className={`${selected === null || selected == undefined ? 'bg-gray-900 cursor-none border-r-2 border-white' : 'bg-yellow-200 hover:bg-yellow-300 cursor-pointer border-transparent'} flex-1 flex items-center py-2 pl-3 pr-10 text-left rounded-l-lg   focus:outline-none  sm:text-sm`}>
                                                <Link to={selected == null ? null : '/game?room_code=' + props.games[selected].room_code} className={`${selected === null || selected == undefined ? 'text-gray-50 hover:text-gray-50 cursor-default' : 'text-yellow-800 hover:text-yellow-900 cursor-pointer'}`} >
                                            ...
                                            <Listbox.Button className='relative'>
                                                <span className='flex h-full items-center pl-2 pr-2 cursor-pointer bg-gray-900 rounded-r-lg'>
                                                    <SelectorIcon className='w-5 h-5 text-gray-50 hover:text-gray-300' aria-hidden='true' />
                                                </span>
                                            </Listbox.Button>
                                        </div>
                        ...
                        <div className=' space-x-7 hidden sm:flex sm:mr-7 items-center'>
                            ...
                            <div className='relative  flex items-center justify-center'>
                                <button type='button' onClick={props.games !== null && props.games !== false ? openModal : null} className='transition duration-500 ease-in-out transform hover:scale-110 pl-2 pr-3 py-1 text-sm  text-gray-500 bg-white border rounded-3xl focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75'>
                                    <SearchIcon className='relative h-5 w-5 pr-1 inline-block text-gray-500'aria-hidden='true'/>
                  					Search
                                </button>
                            </div>
...
export default connect(mapStateToProps, { searchSymbols, logout, currentGames, loadGame, noGame })(MyNavbar)